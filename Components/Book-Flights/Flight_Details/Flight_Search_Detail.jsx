...
function buildExpediaFlightsSearchUrl({
  ...
  const passParts = [`adults:${adults}`, `children:${children}`, `infantinlap:${infantVal}`];
...
export default function Flight_Search_Detail() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    enabled: !!searchTriggered && !!departure_id && !!arrival_id && !!outbound_date,
    queryFn: () =>
      GetSerpFlights({
        engine,
        departure_id,
  ...
  const hasOtherFlights = Array.isArray(other_flight) && other_flight.length > 0;
  const hasSearched = searchTriggered && (departure_id && arrival_id && outbound_date);

  // Flight search API returns total price when adults is passed - no multiplication needed
  const displayPrice = (p) => { const n = Number(p); return isNaN(n) ? 0 : n; };
            ...
            <div className="departure_title_head flex flex-col md:flex-row r md:justify-between p-0  gap-4 m-0">
              <div className="title section_title space-y-1 mb-2">
                ...
                <div className="flex flex-wrap items-center gap-2 mt-2 text-xs md:text-sm text-gray-700">
                  ...
                  {(Number(adults) || Number(children_count) || Number(infant_count)) ? (
                    <span className="inline-flex items-center gap-2 px-3 py-1 rounded sky_yellow">
                      {[Number(adults) > 0 && `${adults} adult${Number(adults) > 1 ? "s" : ""}`, Number(children_count) > 0 && `${children_count} child${Number(children_count) > 1 ? "ren" : ""}`, Number(infant_count) > 0 && `${infant_count} infant${Number(infant_count) > 1 ? "s" : ""}`].filter(Boolean).join(", ")}
                    </span>
                  ) : null}
                  {travel_class && (
                ...
                <div className="departure_body  space-y-4">
                  {!hasSearched && (
                    <div className="py-8 text-center space-y-2">
                      ...
                      <p className="text-sm text-gray-600 max-w-md mx-auto">
                        Enter your departure, destination, dates above and click Search to find flights.
                      </p>
                    </div>
                  )}
                  ...
                  {hasFlights &&
                    flight?.map((item, idx) => {
                      ...
                      console.log(item, ",,,,,,,,,,,,,,,pkjpkpkkpk", item?.flights);
                      ...
                      return (
                        ...
                        >
                            ...
                            <div className="d-block d-lg-none">
                              <div className="departure_item flex justify-end items-center gap-2  p-0 text-right  ">
                                ...
                                <div className="price text-xl md:text-2xl font-semibold  leading-tight fw-bold">
                                  $
                                  {displayPrice(item?.price || data?.data?.flights?.price_insights?.lowest_price).toLocaleString()}
                                </div>
                          ...
                          <div className="col-lg-2 mt-3 p-0">
                            <div className="side_content space-y-1 text-sm text-gray-600  md:text-right">
                              {
                              ...
                              <div className="side_content space-y-1 text-sm text-gray-600  md:text-right">
                                {
                                  ...
                                  item?.flights?.map((item) => {
                                    ...
                                    return (
                                      <React.Fragment key={Math.random().toString(36).substring(2, 15)}>

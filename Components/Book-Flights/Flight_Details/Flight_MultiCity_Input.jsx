...
export default function Flight_MultiCity_Input() {
  ...
  const { data: autoCompleteFirstFromData, isLoading: isLoadingFirstFrom } = useQuery({
  ...
  const { data: autoCompleteFirstToData, isLoading: isLoadingFirstTo } = useQuery({
  ...
  const allAirPortsTO = autoFirstToData.filter((item) => Array.isArray(item.airports))
    .flatMap((item) => item.airports);
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      ...
      const clickedInsideInput = e.target.closest && e.target.closest('.header_input');
  ...
  const removeInput = (index) => {
    ...
    const updatedFormik = (formik.values.flights || []).filter((_, i) => i !== index);
    ...
    <>
      <form onSubmit={formik?.handleSubmit}>
            ...
            return (
              <React.Fragment key={index}>
                <div className="d-none d-lg-block">
                  <div className={`header_input_item header_multiple_input flex flex-col ${formik?.errors?.flights ? "pb-2 pad-b mb-3 mt-4" : "pb-3"} md:flex-row gap-3 md:gap-4`} key={index}>
                    {/* First leg: From input */}
                    <div className="d-block d-lg-none">
                      {/* First leg: Date input */}
                      <div className="flex cross_btn items-center justify-end relativ">
                        <button className="btn  p-0 " type="button" onClick={() => removeInput(index)}><RxCross2 /></button>
                      </div>
                    </div>
                    {/* *********************************** */}
                    <div className="header_input_1 header_in2 relative flex flex-1 gap-2">
                      <div className="header_input header_inpu relative h-12">
                        ...
                        <input
                          ...
                          value={formik?.values?.flights?.[index]?.departure_id || ""}
                          ...
                          onChange={(e) => {
                            ...
                            setFromFirst(value);
                            setShowFirstFromDropdown(value.trim().length > 1 ? index : null);
                            const updated = [...openflight];
                            updated[index] = { ...updated[index], departure_id: value };
                            setopenFlight(updated);
                          }}
                          onFocus={() => {
                            if ((fromFirst || "").trim().length > 1 && autoFirstFromData.length) {
                        ...
                        />
                        {
                          (formik.touched.flights?.[index]?.departure_id && formik?.errors?.flights?.[index]?.departure_id && ((formik?.touched?.flights?.[index]?.departure_id) || formik?.submitCount > 0)) && (
                            <p className="text-red-400  err_p mt-1 mb-1"><span className="g_color">*</span>{formik?.errors?.flights?.[index]?.departure_id}</p>
                          )
                        }
                              ...
                              allAirports.map((airport, sIndex) => {
                                return (
                                  <button
                                    ...
                                    onMouseDown={(e) => {
                                      e.preventDefault();
                                      formik.setFieldValue(`flights[${index}].departure_id`, airport?.id || "");
                                      const updated = [...openflight];
                                      updated[index] = { ...updated[index], departure_id: airport?.id || "" };
                                      setopenFlight(updated);
                                      setFromFirst(airport?.id || "");
                                      setShowFirstFromDropdown(null);
                                      dispatch(
                                        setSearchFlight({
                                          multicity: { OneStartFrom: airport?.id || "" },
                                        }),
                                      );
                                    }}
                      ...
                      <div className="header_input header_inpu relative h-12">
                        ...
                        <input
                          ...
                          value={formik?.values?.flights?.[index]?.arrival_id || ""}
                          ...
                          onChange={(e) => {
                            ...
                            const value = e.target.value;
                            setToFirst(value);
                            setShowFirstToDropdown(value.trim().length > 1 ? index : null);
                            const updated = [...openflight];
                            updated[index] = { ...updated[index], arrival_id: value };
                            setopenFlight(updated);
                          }}
                          onFocus={() => {
                            if ((toFirst || "").trim().length > 1 && autoFirstToData.length) {
                              ...
                              allAirPortsTO.map((airport, sIndex) => {
                                return (
                                  <button
                                    ...
                                    onMouseDown={(e) => {
                                      e.preventDefault();
                                      formik.setFieldValue(`flights[${index}].arrival_id`, airport?.id || "");
                                      const updated = [...openflight];
                                      updated[index] = { ...updated[index], arrival_id: airport?.id || "" };
                                      setopenFlight(updated);
                                      setToFirst(airport?.id || "");
                                      setShowFirstToDropdown(null);
                                      dispatch(
                                        setSearchFlight({
                                          multicity: { OneEndTO: airport?.id || "" },
                                        }),
                                      );
                                    }}
                                    className="w-full text-left px-4 py-2 hover:bg-gray-50 cursor-pointer"
                        ...
                        {
                          (formik?.touched?.flights?.[index]?.arrival_id && formik?.errors?.flights?.[index]?.arrival_id && ((formik?.touched?.flights?.[index]?.arrival_id) || formik?.submitCount > 0)) && (
                            <p className="text-red-400 err_p mt-1 mb-1"><span className="g_color">*</span>{formik?.errors?.flights?.[index]?.arrival_id}</p>
                          )
                        }
                      </div>
                    ...
                    >
                      ...
                      {(() => {
                        const selectedDate = formik?.values?.flights?.[index]?.dateTime;
                        return (
                          <input
                            type="text"
                            readOnly
                            ...
                            placeholder={selectedDate ? format(new Date(selectedDate), "EEE, MMM d") : "Select date"}
                            value={selectedDate ? format(new Date(selectedDate), "EEE, MMM d") : ""}
                          />
                        );
                      })()}
                      {/* ********** */}
                        ...
                        <div className="absolute z-50 bg-white mt-2 shadow-xl rounded-lg p-4">
                          <DayPicker
                            ...
                            onSelect={(date) => {
                              ...
                              const updated = [...openflight];
                              updated[index] = { ...updated[index], dateTime: date };
                              setopenFlight(updated);
                      ...
                      {/* ********** */}
                      {
                        (formik?.touched?.flights?.[index]?.dateTime && formik?.errors?.flights?.[index]?.dateTime && ((formik?.touched?.flights?.[index]?.dateTime) || formik?.submitCount > 0)) && (
                          <p className="text-red-400 err_p mt-1 mb-1"><span className="g_color">*</span>{formik?.errors?.flights?.[index]?.dateTime}</p>
                        )
                      }
                ...
                <div className="d-block d-lg-none">
                  ...
                  <div className={`header_input_item header_multiple_input flex flex-col ${formik?.errors?.flights ? "pb-4" : "pb-3"} md:flex-row gap-3 md:gap-4`} key={index}>
                    {/* *********************************** */}
                    <div className="header_input_1 header_in2 relative flex flex-1 gap-2">
                      <div className="header_input header_inpu relative h-12">
                        <div className="icon absolute inset-y-0 start-3 flex items-center pointer-events-none icon_search">
                          ...
                          <Modal.Body>
                            <div className="header_input relative h-12">
                              ...
                              {formik?.touched?.From && formik?.errors?.From && (
                                <p className="text-red-400 mt-1 mb-1"><span className="g_color">*</span>{formik?.errors?.From}</p>
                              )}
                      ...
                      <div className="header_input header_inpu relative h-12">
                        ...
                        {
                          (formik?.touched?.flights?.[index]?.arrival_id && formik?.errors?.flights?.[index]?.arrival_id && ((formik?.touched?.flights?.[index]?.arrival_id) || formik?.submitCount > 0)) && (
                            <p className="text-red-400 err_p mb-3"><span className="g_color">*</span>{formik?.errors?.flights?.[index]?.arrival_id}</p>
                          )
                        }
                    ...
                    >
                      ...
                      {
                        (formik?.touched?.flights?.[index]?.dateTime && formik?.errors?.flights?.[index]?.dateTime && ((formik?.touched?.flights?.[index]?.dateTime) || formik?.submitCount > 0)) && (
                          <p className="text-red-400 err_p mt-1 mb-2"><span className="g_color">*</span>{formik?.errors?.flights?.[index]?.dateTime}</p>
                        )
                      }
        ...
        <div className="button flex items-center mt-2 flight_search_button  justify-between font-semibold ">
          <Link href={""} className="no-underline font_cross" onClick={() => PlusInput()}>
            Add Flight
          </Link>
          <button type="submit"

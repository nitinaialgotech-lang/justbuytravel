...
export default function Boooking_options({ airlineName = "British Airways",
  ...
  let urlToken = getData.get("_tok");
  if (urlToken === "undefined" || urlToken === "null") {
    urlToken = null;
  }
  const departure_token = urlToken;
  ...
  console.log(BookingOptions, "BookingOptions response");

  const shouldUseSerpBooking =
    !!departure_id && !!arrival_id && !!outbound_date && !!departure_token;

  const serpResults = BookingOptions?.data?.results;
  const serpBookingOptions = serpResults?.booking_options || [];
  const serpSelectedFlight = serpResults?.selected_flights?.[0] || null;
  const serpSelectedSegments = serpSelectedFlight?.flights || [];
  const serpFirstSeg = serpSelectedSegments[0];
  const serpLastSeg = serpSelectedSegments[serpSelectedSegments.length - 1] || null;

  const airlineBookingOptions = serpBookingOptions.filter(
    (opt) =>
  ...
  const otherBookingOptions = serpBookingOptions.filter(
    (opt) => !opt?.together?.airline,
  );

  const normalizedFareOptions =
    airlineBookingOptions.length > 0
      ? airlineBookingOptions.slice(0, 3).map((opt) => {
          const t = opt.together || {};
          const localPrices = Array.isArray(t.local_prices)
            ? t.local_prices
            : [];
          const eurPrice =
            localPrices.find((p) => p.currency === "EUR") || localPrices[0];

          const features = [];
          // Use per-option baggage info
          if (Array.isArray(opt.departing?.baggage_prices)) {
            opt.departing.baggage_prices.forEach((text) =>
              features.push({ text, type: "included", icon: "suitcase" }),
            );
          }
          if (Array.isArray(opt.returning?.baggage_prices)) {
            opt.returning.baggage_prices.forEach((text) =>
              features.push({ text, type: "included", icon: "suitcase" }),
            );
          }

          const bookingUrl =
            opt.together?.booking_request?.url ||
            opt.departing?.booking_request?.url ||
            opt.returning?.booking_request?.url ||
            null;

          return {
            name: t.option_title || t.book_with || "Option",
            priceUsd: t.price,
            priceEur: eurPrice?.price ?? null,
            features,
            bookingUrl,
          };
        })
  ...
  const normalizedOtherAirlines =
    otherBookingOptions.length > 0
      ? otherBookingOptions.map((opt) => {
          const t = opt.together || {};
          const localPrices = Array.isArray(t.local_prices)
            ? t.local_prices
            : [];
          ...
          return {
            name: t.book_with || "Provider",
            priceUsd: t.price,
            priceEur: eurPrice?.price ?? null,
            subtitle: t.option_title || null,
            color: "text-blue-600",
            bookingUrl,
          };
        })
  ...
  const primaryAirlineName =
    airlineBookingOptions[0]?.together?.book_with || airlineName;

  if (shouldUseSerpBooking && isLoading && !serpResults) {
    return (
      <section className="booking-options py-8">
  ...
  if (shouldUseSerpBooking && isError) {
    return (
      <section className="booking-options py-8">
        <div className="container">
            ...
            <div className="bg-white rounded card_rounded border border-gray-200 shadow-sm overflow-hidden">
              ...
              <div className="px-4 md:px-6 py-4">
                <div
                  ...
                  role="group"
                  aria-label={`Book with ${primaryAirlineName}`}
                >
                ...
                {!hideAirlineOptions && (
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-gray-200 rounded-lg overflow-hidden mb-4">
                      {normalizedFareOptions.map((fare, idx) => (
                        <div
                          key={fare.name}
                          className={`bg-white p-4 md:p-5 flex flex-col ${idx < normalizedFareOptions.length - 1
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={() => {
                              if (fare.bookingUrl) {
                                window.open(fare.bookingUrl, "_blank");
                                return;
                              }
                              onContinue?.({ fare, provider: airlineName });
                            }}
              ...
              <div className="px-4 md:px-6 py-4 border-t border-gray-100">
                {normalizedOtherAirlines.map((provider, idx) => (
                  <div
                    key={provider.name}
                    className={`flex flex-wrap items-center justify-between gap-3 py-4 ${idx < normalizedOtherAirlines.length - 1
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          onContinue?.({ provider: provider.name, priceUsd: provider.priceUsd });
                        }}
